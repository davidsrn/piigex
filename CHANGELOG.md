...
- **US region** with nine Tier-1 detectors, all default-on with
  ...
  - `us_ssn`: Social Security Number. stdnum-backed; rejects reserved
    areas (000, 666, 9xx), group 00, serial 0000, and known promotional
    numbers. Accepts hyphenated and compact forms.
  - `us_ein`: Employer Identification Number. stdnum-backed; enforces
    the IRS-assigned prefix table. Hyphenated form only (compact form
    omitted to avoid collisions with `us_rtn` and `us_ssn`).
  - `us_itin`: Individual Taxpayer Identification Number. stdnum-backed;
    enforces 9xx area and the IRS-published middle-group ranges (70-99
    excluding 89 and 93).
  - `us_atin`: Adoption Taxpayer Identification Number. Hand-rolled
    validator because `stdnum.us.atin` is shape-only and would otherwise
    catch every 9-digit string failing SSN/EIN/ITIN. Enforces the
    IRS-reserved middle group 93.
  - `us_ptin`: Preparer Tax Identification Number. stdnum-backed.
    Literal `P` + 8 digits.
  - `us_rtn`: ABA Routing Transit Number. stdnum-backed; ABA weighted
    checksum.
  - `us_npi`: National Provider Identifier. Hand-rolled with
    `stdnum.luhn` over the ISO 7812 issuer prefix `80840` + 9 digits.
